...
// todo: fix 2 pawns out, get 7 choose split, says you don't have a pawn to split with
// todo: fix pawn in end zone, get too high of number, says you already have a pawn there instead of you cannot got that far
// todo: fix get number too high to enter end zone, presents message saying this but still puts pawn in end zone
// todo: get 7 click on pawn in end zone, still brings up option to select split or move 7, when move 7 selected get null pointer exception
// todo: get 8, says you cant go this far (7 to end zone) but still puts pawn in end zone
...
public class Runner {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		JFrame frame1 = new HelpGUI("Help");
		...
		while (!gameOver(p, c)) {
			...
			// Player decides in the GUI what they would like to do and returns either Sorry
			// or an int to move
			...
			// Display the card
			// JOptionPane.showMessageDialog(null, cCard.getValueAsString(), "Computer
			// Card", JOptionPane.PLAIN_MESSAGE);

			// Computer what they would like to do and returns either Sorry or an int to
			// move
			// Also we get which pawn they want to move
			// if(sorry) {
			// board.sorry();
	...
	public static void compLogic(int v, Pawn[] p, Pawn[] c) {
		...
		for (int i = 0; i < 4; i++) {
			if (v == 4 && c[i].getPosition() <= c[i].getStartPosition() + 4) { // if your pawn is within the range
				...
				int closestToHome = 0;
				for (int j = 0; i < 4; i++) {
					if (p[j].getPosition() - c[i].getStartPosition() < closestToHome
							&& p[j].getPosition() - c[i].getStartPosition() != c[i].getPosition()) {// if you pull an 11
																									// bump the player
																									// that has the
																									// smallest distance
																									// from your home
	...
	public static int randMove(int move) {
		...
		int x = ran.nextInt(4);
		return x; // returns a random number- this will be the index of the pawn you choose to
					// move.
	}
	...
	public static Pawn[][] createPawns() {
		...
		String color = (String) JOptionPane.showInputDialog(null, "Choose a color to play as:\n",
				"Sorry! : Color Choice", JOptionPane.PLAIN_MESSAGE, null, colorChoices, "Red");
	...
	public static String getDifficulty() {
		...
		String kindness = (String) JOptionPane.showInputDialog(null,
				"Choose a kindless level for the computer opponent:\n", "Sorry! : Kindness Choice",
				JOptionPane.PLAIN_MESSAGE, null, kindChoices, "Nice");
